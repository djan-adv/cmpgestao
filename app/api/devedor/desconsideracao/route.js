...
async function buscarPorCnpj(token, cnpj) {
  ...
  let r, data
  try {
    r = await fetch(url, { headers: { ...PDPJ_HEADERS, Authorization: 'Bearer ' + token, Accept: 'application/json' }, signal: AbortSignal.timeout(25000) })
...
export async function GET(request) {
  ...
  // 2) para cada processo (até o teto), busca o detalhe e escaneia as movimentações
  ...
  return Response.json({
    ...
    totais: { processos: processos.length, detalhados: alvo.length, sem_acesso: semAcesso, candidatas: candidatas.length, analisadas: paraIA.length },
    achados, processos,
