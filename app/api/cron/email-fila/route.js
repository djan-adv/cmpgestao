...
export async function GET(request) {
  ...
  const fila = await sb.from('emails_agendados')
    .select('id,para,cc,assunto,corpo,numero,tentativas,enviar_em')
  ...
  for (const it of itens) {
    ...
    const r = await enviarEmailCore({ para: it.para, cc: it.cc, assunto: it.assunto, corpo: it.corpo, numero: it.numero })
    if (r.ok) {
