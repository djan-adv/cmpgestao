'use client'
// Chat da Equipe — versão genérica, independente de qualquer outro sistema.
// Visual e uso parecidos com WhatsApp: conversa em grupo ("Todos") e conversas
// privadas entre duas pessoas, com resposta citada, envio de imagens (prints),
// cor por pessoa e alarme (notificação push) no celular mesmo com o app fechado.
//
// Login: e-mail e senha do Supabase Auth (os usuários são criados no painel do
// Supabase — ver LEIA-ME.md). Não guarda nenhuma configuração de escritório:
// tudo que a tela precisa vem das tabelas criadas pelo banco.sql.
...
// Nome curto para o chip da conversa. Só o primeiro nome basta até existirem
// duas pessoas com o mesmo primeiro nome — aí vira "M. Rita" e "M. Eduarda";
// repetindo os dois, cai no nome completo.
...
// Fator de tamanho do texto das conversas. 1 = tamanho normal; aumente para
// 1.25 ou 1.5 se a equipe preferir letras maiores no celular.
const F = 1
...
const CHAVE_ALVO = id => 'chat_alvo_' + id
...
export default function ChatEquipe() {
  ...
  const [imgToken, setImgToken] = useState('') // p/ mostrar as imagens via /api/anexo
  ...
  // token do Supabase para junto. startAutoRefresh()/stopAutoRefresh() é a
  // recomendação oficial do Supabase para apps mobile: renova assim que a tela
  ...
  // chegar: se o contato salvo não existe mais (saiu da equipe), cai em
  ...
  // token p/ exibir as imagens via /api/anexo — atualizado periodicamente
  useEffect(() => {
    if (!euId) return
    let ativo = true
  ...
  useEffect(() => {
    if (!euId) return
    const canal = supabase.channel('chat-equipe')
  ...
  // ——— rolagem ———
  // 'irAoFim' repete o ajuste nos quadros seguintes (pegando imagem e layout),
  // e o auto-scroll das mensagens novas só acontece se você JÁ estava no fim —
  // assim quem subiu para ler algo antigo não é empurrado de volta.
  ...
  const irAoFim = useCallback(() => {
    ...
    setTimeout(fim, 120)
    setTimeout(fim, 400)   // imagens e fontes já carregadas
  ...
  async function enviarPrint(file) {
    if (!file) return
    if (file.size > 8 * 1024 * 1024) { alert('Essa imagem tem mais de 8 MB — reduza e tente de novo.'); return }
    try {
      ...
      if (!up.ok || j.erro) { alert('Não deu pra enviar a imagem: ' + (j.erro || ('HTTP ' + up.status))); return }
      const payload = {
      ...
      if (r.data.id > ultimoIdRef.current) ultimoIdRef.current = r.data.id
      notificarEnvio(payload)
    } catch (e) { alert('Erro ao enviar a imagem: ' + (e && e.message || e)) }
  }
      ...
      <div style={{ minHeight: '100vh', background: VERDE_ESCURO, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: 'system-ui,Arial,sans-serif' }}>
        <form onSubmit={entrar} style={{ background: '#fff', borderRadius: 14, padding: 26, width: '100%', maxWidth: 360, boxShadow: '0 10px 40px rgba(0,0,0,.25)' }}>
          <div style={{ textAlign: 'center', marginBottom: 18 }}>
            <div style={{ fontSize: 22, fontWeight: 700, color: VERDE_ESCURO }}>💬 Chat da Equipe</div>
            <div style={{ fontSize: 13, color: '#667', marginTop: 4 }}>Entre com seu e-mail e senha</div>
          </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', fontFamily: 'system-ui,Arial,sans-serif', background: FUNDO, overflow: 'hidden' }}>
      ...
      <div style={{ background: VERDE_ESCURO, color: '#fff', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        ...
        {pushEstado !== 'indisponivel' && (
          <button onClick={ativarAlarme} disabled={pushEstado === 'ativando' || pushEstado === 'verificando'} title={pushEstado === 'ativado' ? 'Alarme ativado — você recebe notificação mesmo com o app fechado' : 'Ativar alarme (notificação no celular)'}
            style={{ marginLeft: 'auto', background: pushEstado === 'ativado' ? 'rgba(255,255,255,.3)' : 'rgba(255,255,255,.15)', border: 0, color: '#fff', borderRadius: 8, padding: '6px 10px', fontSize: 13, cursor: pushEstado === 'ativando' ? 'default' : 'pointer' }}>
            {pushEstado === 'ativado' ? '🔔 ativado' : pushEstado === 'ativando' ? '🔔 …' : '🔕 ativar alarme'}
          </button>
        )}
        <button onClick={() => setSeletorCor(s => !s)} title="Escolher minha cor" style={{ marginLeft: pushEstado === 'indisponivel' ? 'auto' : 0, background: 'rgba(255,255,255,.15)', border: 0, color: '#fff', borderRadius: 8, padding: '6px 10px', fontSize: 13, cursor: 'pointer' }}>🎨</button>
      ...
      <div style={{ display: 'flex', gap: 6, padding: '8px 10px', overflowX: 'auto', background: '#fff', borderBottom: '1px solid #ddd', flexShrink: 0 }}>
        ...
        {pessoas.map(p => (
          <button key={p.id}
            ...
            style={{ flexShrink: 0, border: alvo && alvo.id === p.id ? '1.5px solid ' + VERDE : '1px solid #ddd', background: alvo && alvo.id === p.id ? '#e7f7f2' : '#fff', color: '#222', borderRadius: 16, padding: '8px 14px', fontSize: fs(12.5), fontWeight: 600, cursor: 'pointer' }}>
            🔒 {rotulos[p.id] || (p.nome || '').split(' ')[0]}</button>
        ))}
      </div>
      ...
      <form onSubmit={enviar} style={{ display: 'flex', gap: 8, padding: '8px 10px', background: '#fff', borderTop: '1px solid #ddd', flexShrink: 0, paddingBottom: 'max(8px, env(safe-area-inset-bottom))' }}>
        ...
        <button type="button" onClick={() => fileInputRef.current && fileInputRef.current.click()} title="Enviar imagem"
        ...
        <input ref={inputRef} value={texto} onChange={e => setTexto(e.target.value)} placeholder="Mensagem ou cole uma imagem"
