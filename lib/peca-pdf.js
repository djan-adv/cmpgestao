...
export async function pecaEmPdf({ texto, processo, rodape, medir }) {
  ...
  // timbre: usa a logo do escritório se ela estiver no projeto
  let logo = null
  try {
    const p = path.join(process.cwd(), 'public', 'logo_cmp_full.png')
    if (fs.existsSync(p)) logo = await pdf.embedPng(fs.readFileSync(p))
  } catch (e) {}
