...
import { fraseTeto } from '../app/api/_lib/planos.js'
...
export async function limiteDoEscritorio(esc, sb) {
  if (!esc || esc === ESCRITORIO_RAIZ) return { bytes: null, emTeste: false }
  try {
    const cli = sb || admin()
    const { data } = await cli.from('escritorios').select('limite_gb,teste_ate').eq('id', esc).maybeSingle()
    const gb = data && data.limite_gb
    const emTeste = !!(data && data.teste_ate)
    if (gb == null || !(Number(gb) > 0)) return { bytes: null, emTeste }
    return { bytes: Math.round(Number(gb) * GB), emTeste }
  } catch (e) {
    ...
    return { bytes: null, emTeste: false }
  }
}
...
export async function cabeMais(esc, bytes, sb) {
  const { bytes: limite, emTeste } = await limiteDoEscritorio(esc, sb)
  ...
  return {
    ...
    erro: fraseTeto({ oque: 'espaço para documentos', limite: gb(limite), emTeste, unidade: ' GB' }),
  }
}
