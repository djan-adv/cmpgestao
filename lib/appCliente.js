...
export const DOCS_ROOT = '/opt/cmpdocs'
...
export function pastaAppCliente(numero) {
  return path.join(DOCS_ROOT, String(numero || '').replace(/\D/g, ''), PASTA_APP_CLIENTE)
}
...
export function copiarParaAppCliente(numero, docNome, buf) {
  try {
    if (!buf || !buf.length) return null
    const dig = String(numero || '').replace(/\D/g, '')
    if (!dig) return null
    const dir = pastaAppCliente(dig)
